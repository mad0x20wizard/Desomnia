...
Where the counters cannot be read at all — an exotic kernel without IO accounting, or a daemon running without the privileges to ask — the threshold is ignored: a warning is logged once, and the group is treated as if the threshold were met, so the watch degrades to whatever the other attributes still measure.
