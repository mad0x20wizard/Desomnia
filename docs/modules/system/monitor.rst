...
The System Monitor coordinates the other monitors but does not observe anything on its own. The :doc:`configuration reference <config>` covers the global event handlers (``onIdle``, ``onUsage``, ``onSuspend``, ``onSuspendTimeout``, ``onResume``) and the ``timeout`` attribute.
